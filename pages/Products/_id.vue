        ...
        <div class="d-flex align-items-center mt-4 mb-5">
          ...
          <div v-for="(item, index) in proCatList" :key="index">
            <p
              ...
              tabindex="0"
              :class="{ TopicActive: CatName == item.eproCatName }"
            >
              {{ item.proCatName }}
            </p>
          </div>
...
<script>
  ...
  export default {
    ...
    head() {
      return {
        title: `${this.CnProductName} - 友照醫療儀器`,
        ...
        //   { hid: 'og:title', property: 'og:title', content: `${this.ProData.proName}` },
        //   {
        //     hid: 'og:image',
        //     property: 'og:image',
    ...
    methods: {
      ...
      filterTopic(CatName) {
        ...
        if (CatName == 'index') {
          ...
          vm.$router.push(`/Products/${CatName}`)
        }
      },
      ...
      getPage(page) {
        ...
        if (page == 1) {
          window.location.replace(`https://www.ucare-medical.com/Products/${vm.CatName}`)
        } else {
          window.location.replace(`https://www.ucare-medical.com/Products/${vm.CatName}?page=${page}`)
        }
      },
      ...
      nextPage() {
        ...
        if (vm.currentPage == 1) {
          ...
          window.location.replace(`https://www.ucare-medical.com/Products/${vm.CatName}?page=${vm.currentPage}`)
        }
      },
    ...
    async asyncData({ $axios, params, route }) {
      ...
      if (getCatName == 'index') {
        let getPro = await $axios.get(
          `https://ucarebackend20231109.azurewebsites.net/product?keyword=&productCatId=&page=${currentPage}`
        )
        return {
          currentPage,
        ...
        let getPro = await $axios.get(
          `https://ucarebackend20231109.azurewebsites.net/product?keyword=&productCatId=1&page=${currentPage}`
        )
        return {
          currentPage,
          ProList: getPro.data.Data,
        ...
        let getPro = await $axios.get(
          `https://ucarebackend20231109.azurewebsites.net/product?keyword=&productCatId=2&page=${currentPage}`
        )
        return {
          currentPage,
          ProList: getPro.data.Data,
        ...
        let getPro = await $axios.get(
          `https://ucarebackend20231109.azurewebsites.net/product?keyword=&productCatId=3&page=${currentPage}`
        )
        return {
          currentPage,
          ProList: getPro.data.Data,
    ...
    created() {
      ...
      vm.$axios.get('https://ucarebackend20231109.azurewebsites.net/ProductCategory').then((response) => {
        if (response.data.Code == 200) {
          ...
          let GetCatID1 = []
          GetCatID1 = vm.proCatList.filter((item) => {
            return item.eproCatName == vm.CatName
          })

          vm.CnProductName = GetCatID1[0].proCatName

          if (vm.hasNewCat) {
            let GetCatID = []
            GetCatID = vm.proCatList.filter((item) => {
              return item.eproCatName == vm.CatName
            })
            ...
            vm.$axios
              .get(`https://ucarebackend20231109.azurewebsites.net/product?keyword=&productCatId=${CatID}&page=${vm.currentPage}`)
              .then((response) => {
                if (response.data.Code == 200) {
